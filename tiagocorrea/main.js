...
$(document).ready(function(){
	...
	initHeaderFooter = function(){
				...
				TweenLite.to(window, 0.7, {scrollTo: {y:0, autoKill:false}, ease: Power3.easeInOut, onComplete: function(){
					...
					$.ajax({ type: "GET",
					    ...
					    success : function(data)
					    {
							...
							document.title = "Tiago Correa. "+new_title;
		...
		$('.newsletterLink').on('click', function(e){
			e.preventDefault();
			$('.Newsletter__modal').addClass('is--visible');
			$('.Newsletter__form #newsletterEmail').focus();
		});

		$('.Newsletter__modal .Newsletter__modal__close').on('click', function(e){
			e.preventDefault();
			$('.Newsletter__modal').removeClass('is--visible');
			$('.Newsletter__form #newsletterEmail').val('').blur();
		});
			...
			if ( scrollTop > wH && !$('.scrolltop').hasClass('showed') ) {
			 ...
			 if (scrollTop > $('.page-container').outerHeight() - window.innerHeight + borderWidth - 60) {
			    ...
			    if (wW > 768) {
				  ...
				  TweenLite.set($('footer .newsletterLink'), { y: - footerHeight * 1 + ((scrollTop - ($('.page-container').outerHeight() - window.innerHeight)) * 1) , ease: Linear.easeNone });
				...
				if (wW > 768) {
					...
					TweenLite.set($('footer .newsletterLink'),{ y: - footerHeight * 1 , ease: Linear.easeNone });
	...
	initPhilo = function () {
		...
		var animationBlockWKN = TweenLite.from('.lien-fort .section-visuel img.brique-page', 0.8, {y:-250, ease: Linear.easeNone, paused: true });
	...
	function initNewsletterForm() {
		const form = new Form('.Newsletter__form')
	}
